...
#### 1. Webhook Failures
- **Symptoms**: Consent changes not triggering tasks
- **Diagnosis**: Check PostgreSQL logs for trigger errors
- **Resolution**: Verify internal API endpoint availability

#### 2. Redis Connection Issues
- **Symptoms**: Tasks created in database but not Redis
- **Diagnosis**: Check Redis connectivity and authentication
- **Resolution**: Verify Redis configuration and network access

#### 3. Task Processing Delays
- **Symptoms**: Tasks stuck in pending status
- **Diagnosis**: Check Python worker logs and Redis queue length
- **Resolution**: Scale worker processes or investigate task failures

### Debug Commands

```bash
# Check consent table
SELECT * FROM newsletter_consents WHERE user_id = 'uuid' ORDER BY created_at DESC;

# Monitor Redis queues
LLEN consent_tasks:2024-01-15

# Check Python tasks
SELECT * FROM python_tasks WHERE status = 'pending' ORDER BY created_at;

# View trigger logs
SELECT * FROM pg_stat_user_functions WHERE funcname = 'notify_consent_change';
```
