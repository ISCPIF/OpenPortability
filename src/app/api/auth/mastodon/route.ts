  ...
  async (request: NextRequest, data: {}) => {
    try {
      ...
      try {
        ...
        logger.logWarning(
          'API',
          'GET /api/auth/mastodon',
          'Redis cache miss or error, falling back to database',
          'anonymous',
          {
      ...
      try {
        ...
        instancesList = instances.map(row => row.instance)
        ...
        try {
          ...
          logger.logInfo(
            'API',
            'GET /api/auth/mastodon',
            'Mastodon instances fetched from DB and cached',
            'anonymous',
            {
          ...
          logger.logWarning(
            'API',
            'GET /api/auth/mastodon',
            'Failed to cache instances in Redis',
            'anonymous',
            {
