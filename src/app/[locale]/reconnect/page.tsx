...
export default function MigratePage() {
  ...
  const [reconnectionStats, setReconnectionStats] = useState<ReconnectionStats | null>(null)
  ...
  const checkTokens = async () => {
    try {
      ...
      if (!data.success) {
        setInvalidTokenProviders(data.missingProviders)
  ...
  useEffect(() => {
    const fetchReconnectionStats = async () => {
      try {
        const response = await fetch('/api/stats/reconnections');
        if (response.ok) {
          const data = await response.json();
          setReconnectionStats(data);
        }
      } catch (error) {
        ...
        <div className="bg-[#2a39a9]">
          ...
          <div className="mt-[600px] bg-[#2a39a9]">
            {isAutomaticReconnect ? (
              ...
              <ReconnexionOptions
                onAutomatic={() => setIsAutomaticReconnect(true)}
                onManual={() => setShowOptions(false)}
