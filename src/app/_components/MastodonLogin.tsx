...
export default function MastodonLogin() {
  ...
  const [selectedInstance, setSelectedInstance] = useState('mastodon');

  const handleMastodonLogin = async () => {
    setIsLoading(true);
    try {
      await signIn(selectedInstance);
    } catch (error) {
  ...
  return (
    ...
    >
      <select
        ...
        className="w-full px-4 py-2 bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-700"
        disabled={isLoading}
      >
        <option value="mastodon">mastodon.social</option>
        <option value="piaille">piaille.fr</option>
      </select>
      ...
      >
        {isLoading ? (
          ...
          <>
            ...
            Continuer avec {selectedInstance === 'piaille' ? 'Piaille' : 'Mastodon'}
          </>
        )}
      </motion.button>
    </motion.div>
