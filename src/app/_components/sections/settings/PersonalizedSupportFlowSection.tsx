...
//   hasMastodonDM: boolean;
//   onDMConsentChange: (type: 'bluesky_dm' | 'mastodon_dm', value: boolean) => Promise<void>;
// }
...
//   console.log("PROPS",userId,blueskyHandle,mastodonHandle,hasPersonalizedSupport,hasBlueskyDM,hasMastodonDM,onDMConsentChange)
...
//   useEffect(() => {
//     // Afficher le composant si personalized_support est true et qu'il y a au moins un handle
//     setShouldShowComponent(hasPersonalizedSupport && (Boolean(blueskyHandle) || Boolean(mastodonHandle)));
//     console.log("Component visibility conditions:", {
//       hasPersonalizedSupport,
//       blueskyHandle,
//       mastodonHandle,
//       shouldShow: hasPersonalizedSupport && (Boolean(blueskyHandle) || Boolean(mastodonHandle))
//     });
//   }, [hasPersonalizedSupport, blueskyHandle, mastodonHandle]);

//   // Fonction pour tester l'envoi de DM
//   const handleTestDM = async (platform: 'bluesky' | 'mastodon') => {
//     const handle = platform === 'bluesky' ? blueskyHandle : mastodonHandle;
//     if (!userId || !handle || testingDM) return;
...
//             {testStatus !== 'testing' && blueskyHandle && (
//               <button
//                 onClick={() => handleTestDM('bluesky')}
//                 disabled={testingDM}
...
//               >
//                 {testStatus === 'idle' && t('notifications.personalizedSupport.actions.testBlueskyDM')}
//                 {testStatus === 'success' && t('notifications.personalizedSupport.actions.testAgainBluesky')}
//                 {testStatus === 'failed' && t('notifications.personalizedSupport.actions.retryBluesky')}
//               </button>
//             )}

//             {testStatus !== 'testing' && mastodonHandle && (
//               <button
//                 onClick={() => handleTestDM('mastodon')}
//                 disabled={testingDM}
//                 className={`${plex.className} px-4 py-2 bg-[#563acc] text-white rounded-full text-sm font-medium hover:bg-[#6b4de6] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2`}
//               >
//                 {testStatus === 'idle' && t('notifications.personalizedSupport.actions.testMastodonDM')}
//                 {testStatus === 'success' && t('notifications.personalizedSupport.actions.testAgainMastodon')}
//                 {testStatus === 'failed' && t('notifications.personalizedSupport.actions.retryMastodon')}
//               </button>
//             )}
