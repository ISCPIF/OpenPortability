...
//         await redis.redisClient.set(key, data.bluesky_username);
...
//         };
//         await redis.redisClient.set(key, JSON.stringify(mastodonData));
...
//     try {
//       const blueskyKeys = await redis.client.keys(`${SocialMappingService.BLUESKY_KEY_PREFIX}*`);
//       const mastodonKeys = await redis.client.keys(`${SocialMappingService.MASTODON_KEY_PREFIX}*`);
